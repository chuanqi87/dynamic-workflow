...
import { createWorktree as createWorktreeShared } from "@workflow/host-support";
...
export class OpencodeAdapter implements HostAdapter {
  ...
  /**
   * Create an isolated git worktree for an agent. Delegates to the shared
   * host-support implementation. If `baseDir` is not a git repo (or git is
   * unavailable), gracefully degrades to running in `baseDir`.
   */
  async createWorktree(baseDir: string, id: string): Promise<{ dir: string; cleanup(): Promise<void> }> {
